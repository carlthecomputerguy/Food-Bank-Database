...
class FoodBankClientGUI:
    def __init__(self, root):
        ...
        self.server_url = "https://database.i70streetreach.com"
    ...
    def setup_gui(self):
        ...
        # Admin-only button for user management
        if self.current_user and self.current_user.get('role') == 'admin':
            ttk.Button(button_frame, text="Manage Users", command=self.open_user_management).pack(side=tk.LEFT, padx=5)
    ...
    def open_user_management(self):
        ...
        # Title
        ttk.Label(main_frame, text="User Management", font=("Arial", 14, "bold")).grid(row=0, column=0, columnspan=2, pady=10)
        
        # User list frame
        list_frame = ttk.LabelFrame(main_frame, text="Existing Users", padding="10")
        list_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), padx=5, pady=5)
        
        # Create treeview for users
        columns = ('Username', 'Full Name', 'Email', 'Role', 'Active')
        user_tree = ttk.Treeview(list_frame, columns=columns, show='headings', height=10)
        ...
        form_frame.grid(row=1, column=1, sticky=(tk.W, tk.E, tk.N, tk.S), padx=5, pady=5)
        
        # Form fields
        row = 0
        ttk.Label(form_frame, text="Username:").grid(row=row, column=0, sticky=tk.W, pady=5)
        username_entry = ttk.Entry(form_frame, width=30)
        username_entry.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=5)
        row += 1
        ...
        fullname_entry.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=5)
        row += 1
        ...
        ttk.Label(form_frame, text="Role:").grid(row=row, column=0, sticky=tk.W, pady=5)
        role_var = tk.StringVar(value='user')
        role_combo = ttk.Combobox(form_frame, textvariable=role_var, values=['user', 'admin'], state='readonly', width=28)
        role_combo.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=5)
        row += 1
        
        def load_users():
            """Load users from server"""
            try:
                response = self.session.get(f"{self.server_url}/api/users", timeout=10)
                if response.status_code == 200:
                    ...
                    for user in users:
                        user_tree.insert('', tk.END, values=(
                            user.get('username') or user.get('Username', ''),
                            user.get('fullname') or user.get('FullName', ''),
                            user.get('email') or user.get('Email', ''),
                            user.get('role') or user.get('Role', ''),
                            'Yes' if (user.get('isactive') or user.get('IsActive')) else 'No'
                        ), tags=(str(user.get('userid') or user.get('UserID')),))
                else:
                    messagebox.showerror("Error", f"Failed to load users: {response.json().get('error', 'Unknown')}")
            except Exception as e:
        ...
        def create_user():
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    error_msg = response.json().get('error', 'Unknown error')
                    messagebox.showerror("Error", f"Failed to create user: {error_msg}")
            except Exception as e:
        ...
        def delete_user():
            ...
            username = user_tree.item(item)['values'][0]
            
            if not messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete user '{username}'?"):
                return
            
            try:
                response = self.session.delete(f"{self.server_url}/api/users/{user_id}", timeout=10)
                ...
                if response.status_code == 200:
                    ...
                    error_msg = response.json().get('error', 'Unknown error')
                    messagebox.showerror("Error", f"Failed to delete user: {error_msg}")
            except Exception as e:
        ...
        ttk.Button(button_frame, text="Refresh", command=load_users).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Close", command=user_window.destroy).pack(side=tk.LEFT, padx=5)
